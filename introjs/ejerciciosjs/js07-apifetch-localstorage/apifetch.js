...
// datosDelProductoRefactorizado("./json/lego-chef.json");
// datosDelProductoRefactorizado("./json/productos.json");
...
/* Se pone AWAIT a las funciones asíncronas, y poner ASYNC a la función */
  ...
  */
    ...
    const datosDelProductoUsandoAsyncYAwaitYTryCatch = async ( url ) =>{
        try{
            ...
            console.log("Ocurrió un problema", exception);
        }
        console.log("He terminado con async, await y try catch");
    }
    datosDelProductoUsandoAsyncYAwaitYTryCatch("https://reqres.in/api/users?page=2");

    const datosTryCatch = async ( url ) =>{
        try{
            const resolve = await fetch(url);
            const carts = await resolve.json();
            console.table(carts[1].products);
        }catch(exception){
            console.log("Ocurrió un problema", exception);
        }
        console.log("He terminado con async, await y try catch");
    }
    datosTryCatch("https://fakestoreapi.com/carts");
